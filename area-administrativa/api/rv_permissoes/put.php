    <?php
        try {
            if (isset($json['pkId']) && is_numeric($json['pkId'])) {
                ...
                $sql = "UPDATE rv_permissoes SET permissao = :permissao, ativo = :ativo WHERE pkId = :pkId";
                $stmt = $conn->prepare($sql);

                $stmt->bindValue(':permissao', isset($json['permissao']) && trim($json['permissao']) !== '' ? trim($json['permissao']) : null);
