    <?php
        try {
            if (isset($json['pkId']) && is_numeric($json['pkId'])) {
                ...
                $sql = "UPDATE rv_vtr SET nome = :nome, consumo = :consumo, ativo = :ativo WHERE pkId = :pkId";
                $stmt = $conn->prepare($sql);

                $stmt->bindValue(':nome', isset($json['nome']) && trim($json['nome']) !== '' ? trim($json['nome']) : null);
            $stmt->bindValue(':consumo', isset($json['consumo']) && trim($json['consumo']) !== '' ? trim($json['consumo']) : null);
