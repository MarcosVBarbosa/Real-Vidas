...
if (!empty($_GET["s"])) {
  $whereS = " AND (cl.razaoSocial LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%' 
                    OR cl.nomeFantasia LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%'
                    OR cl.cpfCnpj LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%'
                    OR cl.cidade LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%'
                    OR cl.pkId = '" . mysqli_real_escape_string($connecta, $_GET["s"]) . "') ";
} else {
  $whereS = "";
}
...
$rs = mysqli_query($connecta, "SELECT COUNT(cl.pkId) totalRegistros 
                              FROM rv_clientes cl
                              WHERE cl.ativo = 'S' $whereS");
              ...
              <div class="card">
                ...
                <div class="card-body table-responsive p-0" style="height: 540px;">
                  <table class="table table-head-fixed table-striped table-bordered text-nowrap table-sm">
                    ...
                    <tbody>
                      <?php
                      $query = "SELECT LPAD(cl.pkId,5,0) pkId,cl.razaoSocial,cl.nomeFantasia,cl.cpfCnpj,cidade
                            FROM rv_clientes cl
                            WHERE cl.ativo = 'S'
                            $whereS
                            ORDER BY cl.razaoSocial
                            LIMIT $inicio,$max";
