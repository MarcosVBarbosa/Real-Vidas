...
if (!empty($_GET["s"])) {
  $whereS = " AND (nome LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%' 
                    OR cidade LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%'
                    OR estado LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%'
...
$rs = mysqli_query($connecta, "SELECT COUNT(pkId) totalRegistros FROM rv_estabelecimentos WHERE ativo = 'S' $whereS");
              ...
              <div class="card">
                ...
                <div class="card-body table-responsive p-0" style="height: 540px;">
                  <table class="table table-head-fixed table-striped table-bordered text-nowrap table-sm">
                    ...
                    <tbody>
                      <?php
                      $query = "SELECT LPAD(pkId,5,0) pkId,nome,cidade,estado
                          ...
                          <tr>
                            ...
                            <td style="padding-right:.75rem">
                              ...
                              if (validButtonSubmit($acessoPermissoes['isPermissao'], 'Remover')) {
                                echo '<a title="Remover" href="javascript:void(0)" 
                                      onclick="if(confirm(\'Deseja realmente excluir esse registro?\')) { window.location=\'remove.php?ref=' . base64_encode($row->pkId) . '\'; }" 
