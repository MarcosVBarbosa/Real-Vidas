...
if (!empty($_GET["s"])) {
  $whereS = " AND (
                    nome LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%' 
                    OR pkId = '" . mysqli_real_escape_string($connecta, $_GET["s"]) . "'
                  ) 
               AND fkPerfil = '" . base64_decode($_GET["ref"])  . "'";
} else {
  $whereS = "AND fkPerfil = '" . base64_decode($_GET["ref"])  . "'";
}
...
$rs = mysqli_query($connecta, "SELECT COUNT(pkId) totalRegistros 
                              FROM rv_acesso 
                              WHERE ativo = 'S' $whereS");
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Lista de cadastros <a href="insert.php?pkPerfil=<?php echo $_GET["ref"] ?>" class="btn btn-primary btn-xs"><i class="fas fa-plus"></i> Novo</a></h3>
                  ...
                  <table class="table table-head-fixed table-striped table-bordered text-nowrap table-sm">
                    ...
                    <tbody>
                      ...
                      $query = "
                            ...
                            FROM 
                              rv_acesso 
                            WHERE rv_acesso.ativo = 'S'
                            $whereS
                            ORDER BY rv_acesso.acesso
                            LIMIT $inicio,$max";
                      ...
                      if (mysqli_num_rows($rs) > 0) {
                        while ($row = mysqli_fetch_object($rs)) { ?>
                          <tr>
                            ...
                            <td><?php echo arrayToString(json_decode($row->permissoes, true)); ?></td>
                            <td style="padding-right:.75rem">
                              <a href="insert.php?ref=<?php echo base64_encode($row->pkId) ?>&pkPerfil=<?php echo $_GET["ref"] ?>" class="btn btn-sm btn-outline-primary"><i class="fas fa-edit"></i></a>
                              <?php
