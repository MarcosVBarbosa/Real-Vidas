...
if (!empty($_GET["s"])) {
  $whereS = "
  AND (
    p.nome LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%'
    OR u.nome LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%' 
    OR u.email LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%'
    OR u.pkId = '" . mysqli_real_escape_string($connecta, $_GET["s"]) . "'
  ) ";
} else {
  $whereS = "";
}
...
$rs = mysqli_query($connecta, "SELECT COUNT(u.pkId) totalRegistros 
                              FROM rv_administradores u
                              WHERE u.ativo = 'S' $whereS");
                ...
                <div class="card-body table-responsive p-0" style="height: 540px;">
                  <table class="table table-head-fixed table-striped table-bordered text-nowrap table-sm">
                    ...
                    <tbody>
                      <?php
                      $query = "
                      SELECT LPAD(u.pkId,5,0) pkId,u.nome,u.email,
                      p.nome perfil
                      FROM rv_administradores u
                      JOIN rv_perfil p ON u.fkPerfil = p.pkId
                      WHERE u.ativo = 'S'
                      $whereS
                      and u.pkId <> 1
                      ORDER BY u.nome
