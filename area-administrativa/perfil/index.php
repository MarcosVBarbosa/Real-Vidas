...
if (!empty($_GET["s"])) {
  $whereS = " AND (
                    rPf.nome LIKE '%" . mysqli_real_escape_string($connecta, $_GET["s"]) . "%' 
                    OR rPf.pkId = '" . mysqli_real_escape_string($connecta, $_GET["s"]) . "'
                  ) ";
} else {
  $whereS = "";
}
...
$rs = mysqli_query($connecta, "SELECT COUNT(pkId) totalRegistros 
                               FROM rv_perfil 
                               WHERE rv_perfil.ativo = 'S' $whereS");
                ...
                <div class="card-body table-responsive p-0" style="height: 540px;">
                  <table class="table table-head-fixed table-striped table-bordered text-nowrap table-sm">
                    ...
                    <tbody>
                      ...
                      $query = "
                            SELECT 
                              LPAD(rPf.pkId,5,0) pkId,rPf.nome, GROUP_CONCAT(rAc.acesso) as acessos
                            FROM 
                              rv_perfil rPf 
                              JOIN rv_acesso rAc ON rAc.fkPerfil = rPf.pkId
                            WHERE 
                              rPf.ativo = 'S'
                              AND rAc.ativo = 'S'
                              AND rPf.pkId <> '00001'
                        ...
                        while ($row = mysqli_fetch_object($rs)) { ?>
                          <tr>
                            ...
                            <td style="padding-right:.75rem">
                              ...
                              <a href="../permissoes/?ref=<?php echo base64_encode($row->pkId) ?>" class="btn btn-sm btn-outline-primary"><i class="fas fa-unlock"></i> </a>
                              <?php
